...
Индексы: `unique(code)` (горячий путь редиректа `WHERE code = ?`),
композитный `(user_id, created_at)` (список ссылок пользователя с сортировкой по дате;
левый префикс покрывает и обычные выборки по `user_id`, и FK).
...
Индексы: композитный `(link_id, created_at)` (журнал переходов ссылки с сортировкой
по дате; левый префикс покрывает выборки по `link_id` и FK).
При удалении ссылки — каскадное удаление кликов.

> **Масштабирование.** Горячие пути (редирект по `code`, список ссылок пользователя,
> журнал кликов ссылки) закрыты индексами выше и не деградируют с ростом таблиц.
> Поиск в таблице по подстроке URL/кода (`LIKE '%...%'`) не может использовать
> btree-индекс из-за ведущего `%`; если такой поиск станет критичным на больших
> объёмах — добавляется GIN-индекс с расширением `pg_trgm`. Счётчик `clicks_count`
> денормализован, поэтому общее число кликов не требует `COUNT(*)` по `clicks`.
